window.addEventListener("DOMContentLoaded" ,function(){
    if(typeof ClassicEditor == 'function') { // 에디터 사용 S
        const {
            loadEditor
        } = commonLib;
...
function callbackFileUpload(files) {
    ...
    const attachTpl = document.getElementById("attache_tpl").innerHTML;
    ...
    for(const file of files) {
        ...
        const dom = domParser.parserFromString(html, "text/html");
